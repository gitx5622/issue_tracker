...
import {FaQuoteRight, FaListUl, GrAdd, AiOutlineWallet, BiStopCircle, GrCompliance, FaRegCheckCircle, FiEdit} from "react-icons/all";
...
const Sidebar = () => {
  ...
  const issues = allIssues.map(issue => { return (
    <div>
          ...
          <Tag style={{float:"right", borderRadius:"10px"}} color="#2db7f5">
            <a href={`${issue.repository.html_url}/issues`}>{issue.number}</a>
          </Tag>
          </ListGroupItem>
        </div>
  )});
